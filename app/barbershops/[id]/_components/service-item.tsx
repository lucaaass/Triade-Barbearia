...
import { addDays, format, setHours, setMinutes, isToday, isAfter } from "date-fns";
...
const ServiceItem = ({ service, barbershop, isAuthenticated }: ServiceItemProps) => {
  ...
  const handleHourClick = useCallback((time: string) => {
    setHour(time);
  }, []);
  ...
  }, [hour, date, data?.user, service.id, barbershop.id, router]);
  ...
  const timeList = useMemo(() => {
    ...
    return generateDayTimeList(date).filter((time) => {
      ...
      const isFuture = isToday(date) ? isAfter(timeDate, now) : true;
      return (
        isFuture &&
                ...
                <SheetContent className="p-0 overflow-auto [&::-webkit-scrollbar]:hidden">
                  ...
                  <div className="py-6">
                    <Calendar
                      ...
                      locale={ptBR}
                      fromDate={new Date()} // Permitir seleção a partir do dia atual
                      styles={{
